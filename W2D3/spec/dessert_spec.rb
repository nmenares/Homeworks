...
describe Dessert do
let(:chef) {double("chef", name: "Marce")}
subject(:dessert) {Dessert.new("cake", 10, chef)}
  ...
  describe "#mix!" do
    it "shuffles the ingredient array" do
      ingredients = ["sugar", "berries", "water", "cheese", "butter"]
  ...
  describe "#serve" do
    it "contains the titleized version of the chef's name"do
      allow(chef).to receive(:titleize).and_return("Chef Marce the Great Baker")
      expect(dessert.serve).to eq("Chef Marce the Great Baker has made 10 cakes!")
    end
  end
